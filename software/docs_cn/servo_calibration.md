...
## 舵机安装
不要默认把 12 个舵机都装在机械中心位置。对本项目使用的四足机构来说：

- 肩关节可近似装在机械中心
- 左右大腿需要向相反方向偏置
- 左右小腿需要更明显地向相反方向偏置

如果把大腿和小腿都装在 `60 deg` 中心位，后续即使标定斜率正确，也会在站立和步态阶段过早撞到 PWM 上下限。

对 `PDI-HV5523MG 120 deg` 舵机，当前建议的预装角度为：
...
更详细的装配逻辑、调齿规则和原因分析，请先阅读 [机器狗组装与舵机初始值设置](机器狗组装与舵机初始值设置.md)。

在完成上述机械零位装配后，再进入本页的软件标定流程。下图演示的是几何参考姿态，而不是要求所有舵机必须装在统一中心角：
